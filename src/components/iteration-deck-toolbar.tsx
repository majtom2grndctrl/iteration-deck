...
import { getIterationDeckStore, isDevelopment, type DeckData } from '../core/store';
import * as styles from './iteration-deck-toolbar.css';
...
export class IterationDeckToolbar {
  ...
  render() {
    ...
    return (
      <div class={styles.toolbar} role="toolbar" aria-label="Iteration Deck Controls">
        {hasMultipleDecks && (
          <div>
            <select 
              class={styles.deckSelector}
        ...
        >
          <svg class={styles.navButtonIcon} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class={styles.slideInfo}>
          {currentSlide && (
            <span class={styles.slideLabel}>{currentSlide.label}</span>
          )}
          <span class={styles.slideCounter}>
